...
export default function Login() {
  ...
  const { signIn, signUp, login, isLoading, authError } = useStore();
      ...
      <div className="glass-panel p-10 rounded-2xl w-full max-w-md relative z-10 animate-slideUp">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-2">
            <InputField
              ...
              required={!isSignUp} // Optional for "demo" signin if we want but sticking to strict for now
            />
            {!isSignUp && (
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full py-3 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl font-bold transition-all shadow-lg shadow-indigo-500/25 flex items-center justify-center gap-2 group"
          >
        ...
        <div className="relative my-8">
          ...
          <div className="relative flex justify-center text-sm">
            <span className="px-2 bg-[#0f1115] text-gray-500 glass-panel">Or continue with</span>
          </div>
        </div>

        {/* Removed functional OAuth, just static placeholders if needed, or keeping it empty for stability */}
        <div className="grid grid-cols-2 gap-4 opacity-50 pointer-events-none">
          <button className="py-3 bg-white/5 border border-white/10 rounded-xl text-white flex items-center justify-center gap-2">
